...
class City:
    ...
    @classmethod #again you need to pay attention to what you need to do based on wireframe. This looks different from the other example because we are grabbing the many associated (landmarks) with the one (city, hence method name). In landmark.py we wanted to include the one (city) associated with each many (landmarks).
